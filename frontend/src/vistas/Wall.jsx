    ...
    <div style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      {list && list.length > 0 ? (
        ...
        <div style={{ textAlign: 'center', padding: '1rem 0', color: 'var(--text-tertiary)', fontSize: '0.875rem' }}>
          Aún no hay usuarios en este podio.
        </div>
      )}
    </div>
  );
          ...
          <div key={post.id} className="card interactive-card" style={{ display: 'flex', flexDirection: 'column', gap: '1rem', animationDelay: `${index * 0.1}s` }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--border-color)', paddingTop: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <div
                      onClick={() => post.userId === user?.id ? navigate('/profile') : navigate(`/user/${post.userId}`)}
                      style={{ fontWeight: '500', fontSize: '0.875rem', cursor: 'pointer', color: 'var(--accent-primary)', textDecoration: 'underline', textDecorationColor: 'transparent', transition: 'text-decoration-color 0.2s' }}
                      onMouseEnter={e => e.currentTarget.style.textDecorationColor = 'var(--accent-primary)'}
                      onMouseLeave={e => e.currentTarget.style.textDecorationColor = 'transparent'}
                    >
                      {post.user}
                    </div>
                    {getUserTopInfo(post.userId) && (
                      <div 
                        onClick={() => setSelectedTopInfo(getUserTopInfo(post.userId))}
                      ...
                      >
                        {getUserTopInfo(post.userId).icon}
                        Top {getUserTopInfo(post.userId).name.split(' ')[0]}
                      </div>
                    )}
                  </div>
              ...
              {(() => {
               ...
               {/* Podios Widget Lateral */}
        <div style={{ width: '300px', flexShrink: 0, position: 'sticky', top: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
